...
| 4 | XM430-W350 | `gripper_joint` | 4, Extended Position Control | 0 |
...
실제 기구의 가동 범위를 Dynamixel Wizard로 측정했다. 관절 1~3에는 양 끝에서 2°씩
안전 여유를 적용했고, 그리퍼에는 새로 측정한 닫힘·열림 끝점을 적용했다.
...
| `gripper_joint` | 320.800781° ~ 514.072266° | 320.800781° ~ 514.072266° | 2305 | 0° ~ 193.271484° |
...
그리퍼는 중앙 관절이 아니므로 별도 기준을 사용한다. 링크 변경 후 SDK에서 직접 읽은
닫힘 `5849 tick`(`514.072266°`)과 열림 `3650 tick`(`320.800781°`)을 그대로 사용한다.
그리퍼 기구는 닫힘에서 음의 방향으로 `193.271484°` 이동해야 한다. 따라서 ID 4는 Extended Position Control mode 4,
ID 1~3은 Position Control mode 3을 사용하며 Drive Mode는 모두 0이다.
사용자 명령은 별도 frame으로 정의해 `닫힘 = 0°`, `열림 = 양수`로 받는다. degree
bridge가 아래의 고정 대응식으로 내부 절대각을 만든다.

```text
모터 signed 목표각 = 514.072265625° - 사용자 열림각
사용자 열림각 = opening_cm / 8.65 × 193.271484375°
controller 목표각 = 모터 signed 목표각 - 202.587890625°
```

따라서 `0 cm`는 항상 signed 모터 각 `514.072266°`(controller `311.484375°`)의 닫힘
목표이고, `8.65 cm`는 signed 모터 각 `320.800781°`(controller `118.212891°`)의 열림
목표다. 현재 위치는 이 계산에 사용하지 않는다.

그리퍼 손가락 간격은 최대 열림에서 8.65 cm로 측정했다. 이에 따라 `GripperCommandCm`
메시지와 변환 노드를 추가해 `0 cm = 닫힘`, `8.65 cm = 안전 최대 열림` 명령을 기존
...
### 5.5 그리퍼 모드·방향 및 Hardware Error 진단

그리퍼의 음수 열림 끝점을 단회전 양수각으로 wrapping하면 회전자 방향은 같아도
기구가 반대 경로로 닫힘 스토퍼를 누르는 문제가 발생했다. ID 4만 Extended Position
Control mode 4로 변경하고 하드웨어 플러그인에서 signed 32-bit Present/Goal Position을
처리하도록 수정했다. Torque를 끈 상태에서 SDK로 양 끝을 직접 읽고, 링크 변경 후
닫힘 `5849 tick`, 열림 `3650 tick`으로 다시 보정했다.

스토퍼를 누른 뒤 Ping에서 모델 번호는 일치하지만 Status Packet Alert가 반환되었다.
기존의 모호한 `ping/model check failed` 로그를 통신 실패, 모델 불일치, status alert로
분리하고 Hardware Error Status(70)를 함께 출력하도록 개선했다. Shutdown 오류가 있으면
원인을 제거하고 모터를 reboot한 뒤 다시 활성화해야 한다.
