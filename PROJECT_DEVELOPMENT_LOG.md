...
| `gripper_joint` | -126.386719° ~ 156.621094° | -126.386719° ~ 156.621094° | 2305 | 0° ~ 283.007813° |
...
그리퍼는 중앙 관절이 아니므로 별도 기준을 사용한다. SDK에서 직접 읽은 닫힘
`1782 tick`(`156.621094°`)과 열림 `-1438 tick`(`-126.386719°`)을 wrapping 없이
그대로 사용한다. 두 값은 회전자
방향만 보면 같은 단회전 위치로 정규화할 수도 있지만, 그리퍼 기구는 닫힘에서 음의
방향으로 `283.007813°` 이동해야 한다. 따라서 ID 4는 Extended Position Control mode 4,
...
```text
모터 signed 목표각 = 156.62109375° - 사용자 열림각
사용자 열림각 = opening_cm / 13.0 × 283.0078125°
controller 목표각 = 모터 signed 목표각 - 202.587890625°
```

따라서 `0 cm`는 항상 signed 모터 각 `156.621094°`(controller `-45.966797°`)의 닫힘
목표이고, `13 cm`는 signed 모터 각 `-126.386719°`(controller `-328.974609°`)의 열림
...
처리하도록 수정했다. Torque를 끈 상태에서 SDK로 양 끝을 직접 읽어 닫힘 `1782 tick`,
열림 `-1438 tick`을 확정했다.
